<!DOCTYPE html>
<html>
<head>
    <title>Kwitansi Pasien Laboratorium</title>
    <style>
       /* @page { size: 4cm 11cm portrait; } */
        body {
            font-size:4px;
            margin:7px;
            /* line-height: 8px; */
        }
        @page {
            margin:0px;
        }
        .page-break {
            page-break-after: always;
        }
        .aturan {
            font-size:15px;
            line-height: 10px;
            /* font-size:10px; */
            font-weight:bold;
        }
        .kanan {
            /* width: 60px; */
            /* text-align:left; */
            /* font-size:10px; */
        }
        tr.border_bottom td {
            border-bottom:1pt solid black;
        }

        div.t {
           text-align: right;
           font-size: 4px;
           margin-top: 0px;
           margin-buttom: 4px;
         }

         div.rs {
           text-align: center;
           font-size: 4px;
           margin-top: -5px;
           text-decoration: underline;
         }

         div.tk {
           text-align: center;
           font-size: 4px;
           margin-top: 2px;
         }
         

        div.n {
           text-align: center;
           font-size: 6px;
           margin-top: -5px;
           text-decoration: underline;
         }

         div.r {
           float: right;
           text-align: right;
           font-size:15px;
           line-height: 25px;
           margin-right: 5px;
         }

         /* div.l {
           float: left;
           text-align: left;
           font-size:15px;
           line-height: 25px;
           margin-left: 3px;
         } */

         div.l {
           text-align: center;
           font-size: 5px;
           margin-top: -5px;
         }

         div.b {
           text-align: center;
           font-size: 4px;
           margin-top: 0px;
         }

         div.i {
           float: left;
           text-align: left;
           font-size:14px;
           line-height: 25px;
           margin-left: 6px;
           margin-top: 7px;
         }

         table, th, td {
  border: 1px solid black;
  border-collapse: collapse;
}

td {
  text-align: center;
}


.widthno{
        width: 15px;
        height: 10px;
    }
  
    .widthno2{
        width: 15px;
        height: 5px;
    }    
  


    * {
  box-sizing: border-box;
}

/* Create two equal columns that floats next to each other */
.column {
  float: left;
  width: 21%;
  padding: 3px;
  height: 18px; /* Should be removed. Only for demonstration */
  margin-top: -2px;
}

.column2 {
  float: left;
  width: 3%;
  padding: 3px;
  height: 18px; /* Should be removed. Only for demonstration */
  margin-top: -2px;
}

.column3 {
  float: left;
  width: 100%;
  padding: 3px;
  height: 18px; /* Should be removed. Only for demonstration */
  margin-top: -2px;
}


.column4 {
  float: left;
  width: 50%;
  padding: 3px;
  height: 18px; /* Should be removed. Only for demonstration */

}

.column5 {
  float: left;
  width: 50%;
  padding: 3px;
  height: 18px; /* Should be removed. Only for demonstration */

}

/* Clear floats after the columns */
.row:after {
  content: "";
  display: table;
  clear: both;

}


.rowl {
        border: 1px solid;
        border-radius: 5px;
        height: 32px;
      }



      div.1unv {
           float: left;
         }

         div.2unv {
           float: right;
         }

    </style>
</head>

<body>
@foreach($label as $i => $label)
    <!-- <div class="i"><b class="text-left"><img style="width:80px;" src="data:image/png;base64," style="height:30px;"></b></div> -->
    <!-- <div class="1unv"><img style="width:50px;" float="left" src="data:image/png;base64," style="height:50px;"></div> -->
    <!-- <div class="l"><b class="text-center"> KEMENTRIAN PENDIDIKAN DAN KEBUDAYAAN<br> RUMAH SAKIT UNIVERSITAS TANJUNGPURA</b></div> -->
    <!-- <div class="b"><b class="text-center"> Jalan Prof. Dr. H. Hadari Nawawi Pontianak 78124 <br> Telp. (0561) 576242, Fax. (0561) 765342</b></div> -->
    <!-- <div class="2unv"><img style="width:50px;" float="right" src="data:image/png;base64," style="height:50px;"></div> -->
    <!-- <hr size="3" width="100%" color="black">   -->
    <!-- <div class="r"><b class="text-right">{{$TANGGAL_MASUK}}</b></div> -->
    <div class="rs"><b style="font-family:sans-serif;" class="text-center">RS UNIVERSITAS TANJUNGPURA</b></div>
    <div class="n"><b style="font-family:sans-serif;" class="text-center">RINCIAN BIAYA LABORATORIUM</b></div>
    <!-- <hr size="3" width="52%" color="black">   -->

    <br>
    <div class="t"><b style="font-family:sans-serif;" class="text-right">Pontianak, {{$TANGGAL_MASUK}}</b></div>
<div class="row rowl">
  <div class="column">
    <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">No RM</b></p>
    <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">Nama</b></p>
    <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">Tanggal Lahir</b></p>
    <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">Dokter</b></p>
    <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">Status</b></p>
    <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">Petugas</b></p>
  </div>
  <div class="column2">
  <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">:</b></p>
    <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">:</b></p>
    <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">:</b></p>
    <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">:</b></p>
    <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">:</b></p>
    <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">:</b></p>
  </div>
  <div class="column3">
  <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">{{$label->NORM}}</b></p>
  <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">{{$label->NAMA}}</b></p>
  <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">{{ $label->TANGGAL_LAHIR}}</b></p>
  <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">{{$DOKTER}}</b></p>
  <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">{{$STATUS}}</b></p>
  <p style="margin-top:0em;margin-bottom:0em;"><b style="font-family:sans-serif;">{{$PETUGAS_LAB}}</b></p>
  </div>
</div>
<br>
<table style="margin:0;margin-left:0px;margin-right:0px;width:100%">
  <tr>
    <th style="font-family:sans-serif;font-size:5px;" class="widthno">No.</th>
    <th style="font-family:sans-serif;font-size:5px;">Jenis Tindakan & Tarif</th>
  </tr>
  @foreach($array as $no => $arrays)
  <tr>
    <td class="widthno2">
    <b style="font-family:sans-serif;"> {{++$no}}.</b> 
    </td>
    <td>
    <b style="font-family:sans-serif;"> {{$arrays}}</b> <br>
    </td>   
  </tr>
  @endforeach
  <tr>
   <td style="font-family:sans-serif;font-size:5px;" class="widthno"><b>Total</b></td>
   <td><b style="font-family:sans-serif;font-size:5px;">Rp. {{$TOTAL_TARIF}}<b></td>
  </tr>
</table>

<div class="tk"><b style="font-family:sans-serif;" class="text-center">- TERIMA KASIH ATAS KUNJUNGAN ANDA -</b></div>

<!-- <div class="row">
  <div class="column4" style="text-align:center;">
    <p><b style="font-family:sans-serif;">{{$PETUGAS_LAB}}</b></p>
    <br>
    <br>
    <br>
    <p><b style="font-family:sans-serif;">(........................................)</b></p>
  </div>
  <div class="column5" style="text-align:center;">
  <p><b style="font-family:sans-serif;">Kasir</b></p>
  <br>
  <br>
  <br>
  <p><b style="font-family:sans-serif;">(........................................)</b></p>
  </div>
</div> -->
@endforeach

</body>
</html>
