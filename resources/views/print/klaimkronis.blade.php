<!DOCTYPE html>
<html>
<head>
    <title>Resep Klaim Kronis</title>
    <style>
        body {
            font-size:10px;
            margin:0px;
            line-height: 15px;
        }
        @page {
            margin:0px;
        }
        .page-break {
            page-break-after: always;
        }
        .aturan {
            font-size:15px;
            line-height: 10px;
            /* font-size:10px; */
            font-weight:bold;
        }
        .kanan {
            /* width: 60px; */
            /* text-align:left; */
            /* font-size:10px; */
        }
        tr.border_bottom td {
            border-bottom:1pt solid black;
        }

        b {
             font-size: 10px;
        }


    </style>
</head>

<body>

    <table border="0" cellpadding="1" cellspacing="0" align="center" style="margin:0;margin-left:13px;margin-top:5px;padding:0; width:200px;">
        <tr style="line-height:8px;" class="border_bottom">
            <td style="text-align:center; width:50px;padding-bottom:3px;padding-top:3px;">
                <img style="width:15px;" src="data:image/png;base64," style="height:30px;">
            </td>
            <td style="text-align:center;padding-bottom:0px;" colspan="4" style="width:70px;">
                <b style="font-family:sans-serif;"><h2 style="text-align:center;margin:0;font-size:9px;">RS UNTAN</h2></b>
            </td>
            <td style="text-align:center; width:50px;padding-bottom:3px;padding-top:3px;">
                <img style="width:15px;" src="data:image/png;base64," style="height:30px;">
            </td> 
        </tr>
        <tr>
          <td colspan="8" style="text-align:center;">
            <ins><b style="font-family:sans-serif;">Kwitansi Pelayanan Obat Kronis</b></ins>
          </td>
        </tr>
        <tr>
            <td colspan="8">
                <b style="font-family:sans-serif;">Trs/Reg : {{$kunjungan->NOMOR}}</b>
            </td>
         </tr>
         <tr>
            <td colspan="8">
                <b style="font-family:sans-serif;"> Tgl/Jam : {{$kunjungan->MASUK}}</b>
            </td>
         </tr>
        @foreach($label as $i => $label)
        <tr>
            <td colspan="8">
                <b style="font-family:sans-serif;"> Nama : {{$label->NAMA}}</b>
            </td>
         </tr>
         <tr>
            <td colspan="8">
                <b style="font-family:sans-serif;"> No.RM : {{$label->NORM}}</b>
            </td>
         </tr>
         <tr>
            <td colspan="8">
                <b style="font-family:sans-serif;"> Tgl.Lahir : {{$label->TANGGAL_LAHIR}}</b>
            </td>
         </tr>
         @endforeach
 
   
    <tr>
        <td colspan="8">
         <b style="font-family:sans-serif;">Unit : {{$ruangan->DESKRIPSI}}</b>
         </td>
    </tr>
    <tr>
         <td colspan="8">
         <b style="font-family:sans-serif;">Dpjp : {{$dokter->NAMA_GELAR}}</b>
         </td>
    </tr>
       
   
    <tr>
        <td colspan="8">
             <b style="font-family:sans-serif;">------------------------------------------------------</b>
        </td>
    </tr>
    <tr>
        <td colspan="2">
         <b style="font-family:sans-serif;">Nama Obat</b>
         </td>
         <td colspan="3">
         <b style="font-family:sans-serif;">Qty</b>
         </td>
         <td colspan="3">
         <b style="font-family:sans-serif;">Harga</b>
         </td>
    </tr>
    <tr>
        <td colspan="8">
             <b style="font-family:sans-serif;">------------------------------------------------------</b>
        </td>
    </tr>
     @foreach($tarif as $i => $tarif)
     <tr>
        <td colspan="2">
         <b style="font-family:sans-serif;">{{$tarif->NAMA}}</b>
         </td>
         <td colspan="3">
         <b style="font-family:sans-serif;">{{$tarif->HASIL_QTY_KRONIS}}</b>
         </td>
         <td colspan="3">
         <b style="font-family:sans-serif;">{{$tarif->HASIL_KLAIM_KRONIS}}</b>
         </td>
    </tr>
      @endforeach
      <tr>
        <td colspan="8">
             <b style="font-family:sans-serif;">------------------------------------------------------</b>
        </td>
    </tr>
    <tr>
       <td colspan="2">
        </td>
        <td colspan="3">
        <b style="font-family:sans-serif;"> Total :</b>
        </td>
        <td colspan="3">
         <b style="font-family:sans-serif;">{{$HASIL_TOTAL_KRONIS}}</b>
        </td>
    </tr>

    <tr>
        <td colspan="2">
         <b style="font-family:sans-serif;"></b>
         </td>
         <td colspan="3">
         <b style="font-family:sans-serif;"></b>
         </td>
         <td colspan="3">
         <b style="font-family:sans-serif;">&nbsp;</b>
         </td>
    </tr>
    <tr>
        <td colspan="2">
         <b style="font-family:sans-serif;"></b>
         </td>
         <td colspan="3">
         <b style="font-family:sans-serif;"></b>
         </td>
         <td colspan="3">
         <b style="font-family:sans-serif;">Petugas</b>
         </td>
    </tr>
    <tr>
        <td colspan="2">
         <b style="font-family:sans-serif;"></b>
         </td>
         <td colspan="3">
         <b style="font-family:sans-serif;"></b>
         </td>
         <td colspan="3">
         <b style="font-family:sans-serif;">&nbsp;</b>
         </td>
    </tr>
    <tr>
        <td colspan="2">
         <b style="font-family:sans-serif;"></b>
         </td>
         <td colspan="2">
         <b style="font-family:sans-serif;"></b>
         </td>
         <td colspan="4">
         <b style="font-family:sans-serif;">--------------------</b>
         </td>
    </tr>
    </table>


    </div>



</body>
</html>
